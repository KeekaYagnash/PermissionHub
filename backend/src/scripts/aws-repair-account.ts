...
async function main(){
 if(process.env.NODE_ENV==='production')throw new Error('AWS account repair is disabled in production.');
 ...
 const { PrismaClient }=await import('@prisma/client'),db=new PrismaClient(),accountId=value('--account-id'),tenantIndex=process.argv.indexOf('--tenant-id'),tenantId=tenantIndex>=0?process.argv[tenantIndex+1]:undefined;
