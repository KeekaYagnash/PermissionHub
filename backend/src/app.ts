...
const sessionStore=env.NODE_ENV==='production'?new RedisStore({client:new Redis(env.REDIS_URL,{lazyConnect:false,maxRetriesPerRequest:2}),prefix:'permissionhub:session:'}):undefined;
app.use(session({name:env.SESSION_COOKIE_NAME,secret:env.SESSION_SECRET,resave:false,saveUninitialized:true,rolling:true,store:sessionStore,cookie:{httpOnly:true,secure:env.NODE_ENV==='production',sameSite:'lax',maxAge:env.SESSION_MAX_AGE_MINUTES*60_000,path:'/'}}));
