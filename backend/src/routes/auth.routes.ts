...
import { developmentUsers,identityDomain,loadDevelopmentRoleGrants,persistDevelopmentIdentity } from '../services/identity-domain.service.js';
...
router.get('/session',asyncHandler(async(req:AuthenticatedRequest,res)=>{req.session.csrfToken??=csrfToken();if(!req.session.user){const selected=configuredDevUser();if(selected)req.session.user=identityDomain.sessionUser(selected,'development')}if(req.session.user)await loadDevelopmentRoleGrants(req.session.user);res.json({data:sessionPayload(req.session.user,req.session.csrfToken)})}));
