...
const sessionPayload=(user:SessionUser|undefined,token:string)=>({authenticated:Boolean(user),user:user?{...user,permissions:authorization.permissions(user,user.activeAccountId)}:undefined,csrfToken:token,authProvider:'development',devAuthAvailable:env.NODE_ENV!=='production'&&env.ENABLE_DEV_AUTH,developmentAuthenticationActive:Boolean(user&&configuredDevUser()),awsConnectionMode:env.AWS_CONNECTION_MODE,demoDataEnabled:env.ENABLE_AWS_DEMO_DATA});
...
router.post('/select-account',asyncHandler(async(req:AuthenticatedRequest,res)=>{requireCsrf(req);const user=requireUser(req),{accountId}=z.object({accountId:z.string()}).parse(req.body);await awsAccountRepository.refreshManualAccounts(user.activeTenantId!);const account=identityDomain.account(user,accountId);if(!account||!authorization.canViewAccount(user,account.id))throw new ApiError(403,'AWS account selection is not permitted.','AWS_ACCOUNT_FORBIDDEN');if(!['CONNECTED','DEGRADED'].includes(account.connectionStatus))throw new ApiError(409,'Only validated AWS accounts can be selected.','AWS_ACCOUNT_NOT_CONNECTED');user.activeAccountId=account.id;req.session.user=user;await save(req);res.json({data:sessionPayload(user,req.session.csrfToken!)})}));
router.post('/clear-account',asyncHandler(async(req:AuthenticatedRequest,res)=>{requireCsrf(req);const user=requireUser(req);user.activeAccountId=undefined;req.session.user=user;await save(req);res.json({data:sessionPayload(user,req.session.csrfToken!)})}));
router.get('/context',asyncHandler(async(req:AuthenticatedRequest,res)=>{const user=requireUser(req);if(user.activeTenantId)await awsAccountRepository.refreshManualAccounts(user.activeTenantId);const accounts=identityDomain.accountsFor(user);if(user.activeAccountId&&!accounts.some(account=>account.id===user.activeAccountId&&['CONNECTED','DEGRADED'].includes(account.connectionStatus))){user.activeAccountId=undefined;req.session.user=user;await save(req)}res.json({data:{tenants:identityDomain.tenantsFor(user),organisations:identityDomain.organisationsFor(user),organisationalUnits:identityDomain.ousFor(user),accounts,activeTenantId:user.activeTenantId,activeAccountId:user.activeAccountId,awsConnectionMode:env.AWS_CONNECTION_MODE}})}));
