...
export class AuthorizationService {
 ...
 canApproveRequest(user:SessionUser,request:ScopedRequest){const self=request.requesterUserId===user.id||(!request.requesterUserId&&request.requester===user.displayName);if(self&&!(env.NODE_ENV!=='production'&&env.ALLOW_DEV_SELF_APPROVAL))return false;if(!request.awsAccountId)return false;if(isLocalProvisioningEnabled())return this.forAccount(user,request.awsAccountId,'canApprove')&&this.rolesForAccount(user,request.awsAccountId).includes('SECURITY_REVIEWER');if(request.approverUserId&&request.approverUserId!==user.id)return false;const stages=(request.requiredApprovalStages??[]).filter(stage=>stage!=='PROVISIONER'),stage=stages[(request.completedApprovalStages??[]).length];if(!stage)return true;const roles=this.rolesForAccount(user,request.awsAccountId);if(stage==='ACCOUNT_APPROVER')return roles.some(role=>['ACCOUNT_APPROVER','OU_ADMIN','ORGANISATION_ADMIN'].includes(role));return roles.includes(stage as AppRole)}
 canProvisionRequest(user:SessionUser,request:ScopedRequest){if(!request.awsAccountId||request.status!=='Approved')return false;if(isLocalProvisioningEnabled())return this.canApproveRequest(user,{...request,status:'Pending approval'});return this.forAccount(user,request.awsAccountId,'canProvision')}
