...
import { buildEncryptedStaticCredentials } from './credential-encryption.service.js';
import { identityDomain } from './identity-domain.service.js';

export interface ManualAccountInput{
 accountName:string;awsAccountNumber:string;accountType:AccountType;defaultRegion:string;connectionType:'LOCAL_DEFAULT_CREDENTIALS'|'ACCESS_KEYS'|'ASSUME_ROLE';readRoleArn?:string;provisionRoleArn?:string;externalIdSecretReference?:string;accessKeyId?:string;secretAccessKey?:string;sessionToken?:string
}
...
export class AwsAccountRepository{
 ...
 private connectionData(tenantId:string,awsAccountId:string|undefined,input:ManualAccountInput,row?:any){
  const credentialData=input.connectionType==='ACCESS_KEYS'&&input.accessKeyId&&input.secretAccessKey?buildEncryptedStaticCredentials({accessKeyId:input.accessKeyId,secretAccessKey:input.secretAccessKey,sessionToken:input.sessionToken}):{};
  return {tenantId,...(awsAccountId?{awsAccountId}:{}),connectionType:input.connectionType,connectionSource:'MANUAL',readRoleArn:input.readRoleArn??row?.readRoleArn,provisionRoleArn:input.provisionRoleArn??row?.provisionRoleArn,externalIdSecretReference:input.externalIdSecretReference??row?.externalIdSecretReference,defaultRegion:input.defaultRegion,connectionStatus:'PENDING',provisioningStatus:'DISABLED',provisioningEnabled:false,...credentialData}
 }
}
