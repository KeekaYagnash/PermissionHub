...
async function main(){
 ...
 if(process.env.ENABLE_AWS_DEMO_DATA!=='true')throw new Error('AWS demo seed data is disabled. Set ENABLE_AWS_DEMO_DATA=true explicitly to seed it.');
 ...
 const organisation=await prisma.awsOrganisation.upsert({where:{tenantId_organisationId:{tenantId:tenant.id,organisationId:'o-exampledev'}},update:{isDemo:true,sourceType:'DEMO_SEED'},create:{id:'org_disraptor_dev',tenantId:tenant.id,organisationId:'o-exampledev',organisationName:'Disraptor Organisation',managementAccountId:'111122223333',connectionStatus:'CONNECTED',isDemo:true,sourceType:'DEMO_SEED'}});
 ...
 for(const [id,ouId,name] of ouDefs)await prisma.awsOrganisationalUnit.upsert({where:{awsOrganisationId_ouId:{awsOrganisationId:organisation.id,ouId}},update:{isDemo:true,sourceType:'DEMO_SEED'},create:{id,awsOrganisationId:organisation.id,ouId,name,fullPath:`/Disraptor/${name}`,isDemo:true,sourceType:'DEMO_SEED'}});
 const accountDefs=[
 ...
 for(const [id,accountId,accountName,ouId,accountType,riskTier] of accountDefs){
  const account=await prisma.awsAccount.upsert({where:{tenantId_accountId:{tenantId:tenant.id,accountId}},update:{isDemo:true,sourceType:'DEMO_SEED'},create:{id,tenantId:tenant.id,awsOrganisationId:organisation.id,ouId,accountId,accountName,accountType,environment:accountType.toLowerCase(),riskTier,region:'af-south-1',connectionType:id==='account_sandbox_dev'?'DEFAULT_CHAIN':'ASSUME_ROLE',connectionStatus:id==='account_sandbox_dev'?'CONNECTED':'DISCONNECTED',readRoleArn:`arn:aws:iam::${accountId}:role/PermissionHubReadRole`,provisionRoleArn:`arn:aws:iam::${accountId}:role/PermissionHubProvisionRole`,isDemo:true,sourceType:'DEMO_SEED'}});
  await prisma.awsAccountConnection.upsert({where:{awsAccountId:account.id},update:{connectionSource:'DEMO_SEED'},create:{tenantId:tenant.id,awsAccountId:account.id,connectionType:id==='account_sandbox_dev'?'LOCAL_DEVELOPMENT':'ORGANISATION_MEMBER',connectionSource:'DEMO_SEED',readRoleArn:`arn:aws:iam::${accountId}:role/PermissionHubReadRole`,provisionRoleArn:`arn:aws:iam::${accountId}:role/PermissionHubProvisionRole`,defaultRegion:'af-south-1',organisationsRegion:'us-east-1',connectionStatus:id==='account_sandbox_dev'?'CONNECTED':'PENDING',provisioningStatus:'DISABLED',provisioningEnabled:false}});
 }
 const users=[
