...
export function shouldAutoSelectAccount(context?:AppContext){return Boolean(context&&!(context.activeAwsAccountRecordId??context.activeAccountId)&&context.accounts.length===1&&['CONNECTED','DEGRADED'].includes(context.accounts[0]!.connectionStatus))}
