...
import { cognitoEnabled,logoutCognito } from '../lib/cognito';
...
function UserMenu({compact=false}:{compact?:boolean}){const session=useAuthStore(state=>state.session),clear=useAuthStore(state=>state.clear),navigate=useNavigate();async function logout(){try{if(cognitoEnabled())logoutCognito();else await api.logout()}finally{clear();if(!cognitoEnabled())navigate('/login',{replace:true})}}return <div className={compact?'user-menu compact':'user-menu'}><span><strong>{session?.user?.displayName}</strong><small>{session?.user?.email}</small></span><button onClick={()=>void logout()} title="Sign out" aria-label="Sign out"><LogOut size={15}/></button></div>}
