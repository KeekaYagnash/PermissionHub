...
export function PageHeader({title,description,eyebrow,actions}:{title:string;description?:string;eyebrow?:string;actions?:ReactNode}){return <div className="page-header"><div>{eyebrow&&<p className="eyebrow">{eyebrow}</p>}<h1>{title}</h1>{description&&<p>{description}</p>}</div>{actions&&<div className="page-actions">{actions}</div>}</div>}
...
export function RiskBadge({risk}:{risk?:string}){const value=risk??'Moderate';return <span className={`risk ${value.toLowerCase()}`}>{value}</span>}
export function EnvironmentBadge({environment}:{environment?:string}){const value=(environment??'SANDBOX').replaceAll('_',' ');return <span className={`environment-badge ${value.toLowerCase().replaceAll(' ','-')}`}>{value}</span>}
export function ConnectionBadge({status}:{status?:string}){const value=(status??'Pending').replaceAll('_',' ');return <span className={`connection-badge ${value.toLowerCase().replaceAll(' ','-')}`}><i/>{value}</span>}
...
export function EmptyState({title,body,action}:{title:string;body:string;action?:ReactNode}){return <div className="empty"><Search size={24}/><strong>{title}</strong><span>{body}</span>{action}</div>}
export function ErrorState({title,body,onRetry,technical}:{title:string;body:string;onRetry?:()=>void;technical?:ReactNode}){return <div className="empty error-state"><AlertTriangle size={24}/><strong>{title}</strong><span>{body}</span>{onRetry&&<Button variant="secondary" onClick={onRetry}>Retry</Button>}{technical&&<details className="technical-details compact"><summary>Technical details <ChevronDown size={14}/></summary><div className="technical-details-body">{technical}</div></details>}</div>}
...
export function SectionHeader({title,description,meta,actions}:{title:string;description?:string;meta?:ReactNode;actions?:ReactNode}){return <div className="section-heading"><div><h2>{title}</h2>{description&&<p>{description}</p>}</div>{meta&&<span>{meta}</span>}{actions}</div>}
export function TechnicalDetails({title='Technical details',summary,children,open=false}:{title?:string;summary?:ReactNode;children:ReactNode;open?:boolean}){return <details className="technical-details" open={open}><summary><strong>{title}</strong>{summary&&<small>{summary}</small>}<ChevronDown size={16}/></summary><div className="technical-details-body">{children}</div></details>}
