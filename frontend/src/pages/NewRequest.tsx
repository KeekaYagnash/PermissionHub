...
const steps=['AWS environment','Access subject','Target identity','Permissions','Scope access','Timing and justification','Review'];
...
export default function NewRequest(){
 ...
 useEffect(()=>{setTarget(undefined);setIdentitySearch('');if(targetType)setMaxUnlocked(current=>Math.min(current,3))},[targetType]);
 useEffect(()=>{if(step>3&&!target)setMaxUnlocked(current=>Math.min(current,3))},[target,step]);
 ...
 function stepValid(n:number){
  if(n===1)return Boolean(activeAccount);
  if(n===2)return Boolean(targetType);
  if(n===3)return Boolean(target);
  if(n===4)return selectedPermissionItems().length>0&&(!customModeActive()||policyJsonValidation.valid);
  if(n===5)return scopeType==='ALL'||(scopeType==='RESOURCE'&&selectedResources.length>0)||(scopeType==='ARN'&&arnValidation.arns.length>0&&arnValidation.errors.length===0);
  if(n===6)return fieldErrors().length===0;
  return true;
 }
 ...
 return <div className="page request-workflow">
  ...
  <div className="wizard">
   ...
   <section className="panel request-panel" ref={panelRef}>
    {step===1&&<StepSection title="Confirm AWS environment" help="Every identity, policy and resource lookup is isolated to this authorised AWS account context.">
    ...
    {step===2&&<StepSection title="Choose access subject" help="Human workforce access will use Identity Center when enabled. This release supports IAM roles and legacy IAM users.">
    ...
    {step===3&&<StepSection title={`Choose ${targetType==='USER'?'IAM user':'IAM role'}`} help="Search identities loaded only from the selected AWS account. Role listing uses ListRoles pagination and hides service-linked roles by default.">
    ...
    {step===4&&<StepSection title="Choose permissions" help="Request an AWS-managed policy, modify existing access by creating a customer-managed proposal, or build a custom policy.">
    ...
    {step===5&&<StepSection title="Scope access" help="Select all applicable resources, choose discovered AWS resources, or enter one or more specific ARNs.">
    ...
    {step===6&&<StepSection title="Timing and justification" help="Timing controls how long approved access remains active. Managed-policy attachments are expired by backend grant cleanup metadata, not by editing AWS-managed policies.">
    ...
    {step===7&&<StepSection title="Review permission request" help="Resolve blocking validation issues before submission. The proposed AWS operation will run only after approval and provisioning.">
    ...
    <footer className="wizard-actions"><button onClick={goBack} disabled={step===1}>Back</button>{step<7&&<button onClick={goNext} disabled={!canNext}>Continue</button>}</footer>
   </section>
  </div>
 </div>;
}
