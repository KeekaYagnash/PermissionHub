  ...
  <div className="wizard">
   ...
   <section className="panel request-panel" ref={panelRef}>
    ...
    {step===6&&<StepSection title="Review permission request" help="Resolve blocking validation issues before submission. The proposed AWS operation will run only after approval and provisioning.">
     ...
     <div className="review-grid"><SummaryCard title="AWS account"><p>{activeAccount?.accountName}</p><small>{activeAccount?.accountType.replaceAll('_',' ')} · {activeAccount?.accountId}</small></SummaryCard><SummaryCard title="Access target"><p>{target?.type} · {target?.name}</p><code>{target?.arn}</code></SummaryCard><SummaryCard title="Permissions"><p>{selectedPolicies.map(policy=>policy.policyName).join(', ')||selectedActions.join(', ')}</p><span className={`risk ${riskForPolicies(selectedPolicies).toLowerCase()}`}>{riskForPolicies(selectedPolicies)}</span></SummaryCard><SummaryCard title="Resources"><p>{scopeType==='ALL'?'All applicable resources':selectedResources.map(resource=>resource.name).join(', ')||arnValidation.arns.join(', ')}</p></SummaryCard><SummaryCard title="Duration"><p>{details.duration}</p><small>Expiry: {expiresAt?formatDate(expiresAt):'Permanent'}</small></SummaryCard><SummaryCard title="Reason"><p>{details.justification}</p><small>{details.priority} · {details.approver}</small></SummaryCard><SummaryCard title="Approval path"><p>{activeAccount?.accountType==='PRODUCTION'?'Account approver → Security review → Provisioning':'Account approver → Provisioning'}</p></SummaryCard></div>
