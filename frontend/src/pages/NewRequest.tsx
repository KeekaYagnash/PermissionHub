...
import { PageHeader, RiskBadge, TechnicalDetails } from '../components/ui';

const steps=['Target','Permission','Resources','Access duration','Reason','Review'];
...
export default function NewRequest(){
 ...
 const canNext=step<7?stepValid(step):validation.blocking.length===0;

 function stepValid(n:number){
  if(n===1)return Boolean(activeAccount&&targetType);
  if(n===2)return Boolean(target)&&selectedPermissionItems().length>0&&(!customModeActive()||policyJsonValidation.valid);
  if(n===3)return scopeType==='ALL'||(scopeType==='RESOURCE'&&selectedResources.length>0)||(scopeType==='ARN'&&arnValidation.arns.length>0&&arnValidation.errors.length===0);
  if(n===4)return !fieldErrors().some(error=>/duration|expiry|start|approval/i.test(error));
 ...
 return <div className="page request-workflow">
  ...
  <div className="wizard">
   ...
   <section className="panel request-panel" ref={panelRef}>
    {step===1&&<StepSection title="Who needs access?" help="Confirm the selected AWS account, then choose whether access should be attached to an IAM role or legacy IAM user.">
     ...
     <div className="choice-grid" role="radiogroup" aria-label="Target type">
      ...
      <SelectableCard selected={targetType==='USER'} icon={<UserRound/>} title="Legacy IAM user" help="Attach permissions directly to a named IAM user." onClick={()=>setTargetType('USER')}/>
     </div>
     <p className="muted">Workforce user and group targets are available when the Identity Center integration is enabled and configured.</p>
    </StepSection>}

    {step===2&&<StepSection title="Choose target and permissions" help="Pick the AWS identity, then select the access to request. Technical policy details remain available without dominating the selection flow.">
    ...
    {step===3&&<StepSection title="Choose resources" help="Specific resources are preferred where discovery supports them. Use all resources only when the permission requires wildcard scope.">
    ...
    {step===4&&<StepSection title="Choose access duration" help="Temporary access is the default. Permanent access has no automatic expiry and may require additional review.">
    ...
    {step===5&&<StepSection title="Why do you need this access?" help="Explain the task, project, or incident requiring access. This is what approvers will read first.">
     <div className="form-grid labeled"><Field label="Request title" help="Example: S3 read access for finance reconciliation" error={details.title&&details.title.length<5?'Use at least 5 characters.':undefined}><input value={details.title} onChange={event=>setDetails({...details,title:event.target.value})} placeholder="S3 read access for finance reconciliation"/></Field><Field label="Priority"><select value={details.priority} onChange={event=>setDetails({...details,priority:event.target.value as RequestPriority})}>{priorityOptions.map(item=><option key={item.value} value={item.value}>{item.value} - {item.hint}</option>)}</select></Field><Field label="Approver" error={!details.approver?'Choose an approver.':undefined}>{approvers.data?.length?<select value={details.approver} onChange={event=>setDetails({...details,approver:event.target.value})}><option value="">Select approver</option>{(approvers.data??[]).map((approver:any)=><option key={approver.id} value={approver.name}>{approver.name}</option>)}</select>:<div className="warning">No approvers are configured in the application data.</div>}</Field><Field label="Business justification" help={`Example: Required to troubleshoot the production API deployment. ${details.justification.length} characters used.`} error={details.justification&&details.justification.length<20?'Provide a stronger justification.':undefined} full><textarea value={details.justification} onChange={event=>setDetails({...details,justification:event.target.value})} placeholder="Required to troubleshoot the production API deployment."/></Field><Field label="Optional notes" full><textarea value={details.notes} onChange={event=>setDetails({...details,notes:event.target.value})}/></Field></div>
    </StepSection>}
    ...
    {step===6&&<StepSection title="Review permission request" help="Resolve blocking validation issues before submission. The proposed AWS operation will run only after approval and provisioning.">
     ...
     <div className="review-grid"><SummaryCard title="AWS account"><p>{activeAccount?.accountName}</p><small>{activeAccount?.accountType.replaceAll('_',' ')} · {activeAccount?.accountId}</small></SummaryCard><SummaryCard title="Access target"><p>{target?.type} · {target?.name}</p><code>{target?.arn}</code></SummaryCard><SummaryCard title="Permissions"><p>{selectedPolicies.map(policy=>policy.policyName).join(', ')||selectedActions.join(', ')}</p><RiskBadge risk={riskForPolicies(selectedPolicies)}/></SummaryCard><SummaryCard title="Resources"><p>{scopeType==='ALL'?'All applicable resources':selectedResources.map(resource=>resource.name).join(', ')||arnValidation.arns.join(', ')}</p></SummaryCard><SummaryCard title="Duration"><p>{details.duration}</p><small>Expiry: {expiresAt?formatDate(expiresAt):'Permanent'}</small></SummaryCard><SummaryCard title="Reason"><p>{details.justification}</p><small>{details.priority} · {details.approver}</small></SummaryCard><SummaryCard title="Approval path"><p>{activeAccount?.accountType==='PRODUCTION'?'Account approver → Security review → Provisioning':'Account approver → Provisioning'}</p></SummaryCard></div>
