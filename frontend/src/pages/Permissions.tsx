...
export default function Permissions(){
 ...
 const displayed=useMemo(()=>{
  ...
  if(service)data=data.filter(policy=>policy.services.includes(service));
  if(accessLevel)data=data.filter(policy=>policy.accessLevels.includes(accessLevel));
  return data;
 },[rows,debouncedSearch,service,accessLevel]);
 const services=useMemo(()=>[...new Set(rows.flatMap(p=>p.services))].filter(Boolean),[rows]);
 const columns:TableColumn<IamPolicySummary>[]=[
  {key:'name',header:'Policy name',priority:'high',render:policy=><><strong>{policy.policyName}</strong><small className="mono wrap-cell">{policy.arn}</small></>},
  {key:'type',header:'Type',priority:'medium',render:policy=>policy.type.replace('_',' ')},
  {key:'services',header:'Services',priority:'medium',render:policy=><span className="truncate-cell">{policy.services.join(', ')||'Analysing'}</span>},
  {key:'access',header:'Access levels',priority:'medium',render:policy=><span className="truncate-cell">{policy.accessLevels.join(', ')||'Analysing'}</span>},
  ...
  {key:'attached',header:'Attached',priority:'medium',align:'right',render:policy=>policy.attachmentCount},
  {key:'updated',header:'Last updated',priority:'low',render:policy=>fmt(policy.updatedAt)}
 ];
 return <div className="page">
  <div className="page-header"><div><p className="eyebrow">Permissions</p><h1>IAM managed policy catalogue</h1><p>Live IAM policy metadata loads progressively. Policy documents and detailed risk analysis load only when a policy is selected.</p></div></div>
  <div className="tabs"><button className={scope==='AWS_MANAGED'?'active':''} onClick={()=>setScope('AWS_MANAGED')}>AWS-managed policies</button><button className={scope==='CUSTOMER_MANAGED'?'active':''} onClick={()=>setScope('CUSTOMER_MANAGED')}>Customer-managed policies</button></div>
  <div className="filter-row"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search policy name, ARN, description"/><select value={service} onChange={e=>setService(e.target.value)}><option value="">All services</option>{services.map(s=><option key={s}>{s}</option>)}</select><select value={accessLevel} onChange={e=>setAccessLevel(e.target.value)}><option value="">All access levels</option>{['Read','Write','Permissions management','Unknown'].map(x=><option key={x}>{x}</option>)}</select></div>
  <div className="catalogue-progress" role="status">{loadingInitial?'Loading policy catalogue…':`${displayed.length} shown · ${loadedCount||rows.length}${isComplete?' policies loaded':' policies loaded so far'}`}{loadingMore&&<span>Loading more policies…</span>}{cacheStatus&&<span>Cache: {cacheStatus}</span>}{firstRenderMs!==undefined&&<span>First render: {firstRenderMs}ms</span>}</div>
  {slow&&<p className="warning">IAM is taking longer than expected. Loaded policies remain usable while the catalogue continues refreshing.</p>}
  {error&&<p className="warning">{error} <button onClick={()=>{catalogueStore.delete(cacheKey);setReloadKey(key=>key+1)}}>Retry</button></p>}
  <section className="split">
   {loadingInitial&&!rows.length?<LoadingSkeleton rows={6}/>:<ResponsiveTable rows={displayed} columns={columns} getRowKey={policy=>policy.arn} selectedKey={selected} onRowClick={policy=>setSelected(policy.arn)} emptyTitle="No policies found" emptyBody="Try a different search, service, or access-level filter." rowActionLabel="Inspect policy"/>}
   <aside className="panel policy-detail">{detail.isLoading?<><h2>Loading policy details</h2><LoadingSkeleton rows={4}/></>:detail.data?<><h2>{detail.data.policyName}</h2><p>{detail.data.description}</p><button onClick={()=>navigate(`/new-request?policyArn=${encodeURIComponent(detail.data.arn)}`)}>Request this policy</button><dl><div><dt>ARN</dt><dd className="mono">{detail.data.arn}</dd></div><div><dt>Current version</dt><dd>{detail.data.currentVersion}</dd></div><div><dt>Attached users</dt><dd>{detail.data.attachedUsers.join(', ')||'None'}</dd></div><div><dt>Attached roles</dt><dd>{detail.data.attachedRoles.join(', ')||'None'}</dd></div></dl><h3>Risk observations</h3><div className="chips">{detail.data.observations.map(x=><span key={x}>{x}</span>)}</div><h3>Allowed actions</h3><div className="code-list">{detail.data.actions.map(a=><code key={a}>{a}</code>)}</div><h3>Raw policy JSON</h3><pre>{JSON.stringify(detail.data.document,null,2)}</pre></>:<p className="muted">Select a policy to inspect statements, actions, resource scope, attachments, and detailed risk observations.</p>}</aside>
  </section>
 </div>;
}
